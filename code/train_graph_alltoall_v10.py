...
class GraphAllToAllV10(nn.Module):
    """
    V10 (cIRM, n_fft=1024) + all-to-all adjacency override.
    Overrides stored per-scene adj with fully-connected graph at runtime.
    ...
    def forward(self, Y: torch.Tensor, A: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        if self.use_graph:
            ...
            nb_logmag = self._neighbor_logmag(X, A_used)
            feat      = torch.cat([self_feat, nb_logmag], dim=1)
        else:
            A_used = A
            feat   = self_feat

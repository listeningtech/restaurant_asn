#!/usr/bin/env python3
"""
train_graph_cirm_v10.py

GraphUNet V10: Complex Ideal Ratio Mask (cIRM) + n_fft=1024 + input cross-node.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Mask type comparison (oracle ceilings, this dataset)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  IRM  (sigmoid, mix phase, n_fft=512)  : 13.1 dB  ← V8
  PSM  (tanh,    mix phase, n_fft=1024) : 15.7 dB  ← V9
  cIRM (complex mask,       n_fft=1024) : 112  dB  ← V10 ceiling (theoretical)

cIRM reconstructs the target spectrum exactly:
  output_spec = (Mr + j*Mi) * X_ref
  where Mr + j*Mi = Xs / Xm  (complex ratio)

The ceiling is theoretically unbounded because the model can fully
reconstruct the phase of the target, not just approximate it.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
V10 changes from V9  (four changes total)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
1. Output layer: Conv2d(base, 1, 1) → Conv2d(base, 2, 1)
   Two channels: real and imaginary parts of the complex mask.

2. Mask compression: C * tanh(net)
   The raw cIRM has heavy tails (±hundreds), but 98.5% of values
   fall within [−2, 2].  Compressing with C=10*tanh(net/10) keeps
   the gradient healthy while covering the full dynamic range.
   --mask_scale controls C (default 10.0).

3. Mask application: complex multiply
   Mr, Mi = C*tanh(net).chunk(2)
   Yhat_spec = (Mr + j*Mi) * X_ref

4. Loss: unchanged — SI-SDR on waveform.
   No phase supervision needed; the waveform loss implicitly
   drives the model to predict the correct target phase.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Example (graph ON):
  CUDA_VISIBLE_DEVICES=0 python train_graph_cirm_v10.py \\
    --data /home/rrame12/Desktop/Datasets/RestaurantSim_v4_m1_varspk \\
    --out  /home/rrame12/Desktop/Research/ASN/runs_cirm_v10_on \\
    --batch 8 --epochs 120 --lr 3e-4 --patience 20 \\
    --crop_s 2.0 --nfft 1024 --hop 256 --num_workers 6 \\
    --base 32 --depth 4 --graph_dim 128 --graph_heads 4

Example (graph OFF / strict local-only baseline):
  CUDA_VISIBLE_DEVICES=0 python train_graph_cirm_v10.py \\
    --data /home/rrame12/Desktop/Datasets/RestaurantSim_v4_m1_varspk \\
    --out  /home/rrame12/Desktop/Research/ASN/runs_cirm_v10_off \\
    --batch 8 --epochs 120 --lr 3e-4 --patience 20 \\
    --crop_s 2.0 --nfft 1024 --hop 256 --num_workers 6 \\
    --base 32 --depth 4 --disable_graph
"""
...
class GraphCIRMV10(nn.Module):
    """
    V10: Complex IRM (cIRM).  Uncapped SI-SDR ceiling (~112 dB oracle).

    Graph ON:
        Neighbor log-magnitude injected at full (F, T) resolution as extra
        input channels (primary cross-node signal, same as V8/V9).
        Bottleneck-level cross-node FiLM for high-level conditioning.

    Graph OFF (--disable_graph):
        Standard local-only model.

    Mask:
        Two-channel output: (Mr, Mi) = mask_scale * tanh(net)
        Applied as complex multiply:  Yhat = (Mr + j*Mi) * X_ref
        mask_scale=10 covers >98.5% of oracle cIRM values (range [-2,2] in practice).

    Input channel counts:
        Graph ON  : 4*M  (self: logmag, re/|X|, im/|X|) + M (nb logmag)
        Graph OFF : 3*M
    """
    def __init__(
        self,
        ...
        use_graph: bool   = True,
        mask_scale: float = 10.0,   # V10: cIRM compression constant C
    ):
        super().__init__()
        assert depth >= 2
        ...
        # V10: 2-channel output for real and imaginary mask components
        self.out_mask = nn.Conv2d(base, 2, 1)
    ...
    def forward(self, Y: torch.Tensor, A: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        # V10: cIRM — two-channel (Mr, Mi), compressed with mask_scale*tanh
        M    = self.mask_scale * torch.tanh(self.out_mask(h))  # (BK, 2, F, TT)
        Mr   = M[:, 0]                                          # (BK, F, TT) real part
        Mi   = M[:, 1]                                          # (BK, F, TT) imag part

        # Complex multiply:  (Mr + j*Mi) * X_ref
        Yr   = Mr * X_ref.real - Mi * X_ref.imag
        Yi   = Mr * X_ref.imag + Mi * X_ref.real
        Yhat_spec = torch.complex(Yr, Yi)                      # (BK, F, TT) complex
...
def main():
    ...
    ap.add_argument("--mask_scale",  type=float, default=10.0,
                    help="cIRM compression constant C: output = C*tanh(net)")

#!/usr/bin/env python3
"""
train_graph_psm_v9.py

GraphUNet V9: Phase-Sensitive Mask (PSM) + n_fft=1024 + input cross-node.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Why V8 saturates at ~12.5 dB
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
V8 uses an IRM (magnitude ratio mask, sigmoid output, mix phase):
  output = sigmoid(net) * X_ref

Measured oracle ceilings on this dataset:
  IRM  (n_fft=512,  sigmoid, mix phase) : 13.1 dB   ← V8 ceiling
  IRM  (n_fft=1024, sigmoid, mix phase) : 13.7 dB   (+0.7 dB, bigger window)
  PSM  (n_fft=1024, tanh,    mix phase) : 15.7 dB   (+2.6 dB above V8 ceiling)

V8 model output was only 0.36 dB below its own IRM ceiling — the
architecture is not the bottleneck, the mask type is.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
V9 changes from V8  (three lines total)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
1. n_fft 512→1024, hop 128→256  (better freq resolution, +0.7 dB ceiling)
2. sigmoid→tanh on output mask  (allows negative mask values)
3. Output mask clipped to [-1,1] via tanh  (PSM range)

PSM formula:
  mask(f,t) = (|S(f,t)| / |M(f,t)|) * cos(φ_S(f,t) − φ_M(f,t))
  range: [−1, 1]

Bins where target and mixture are in phase  (cos > 0) → positive mask
Bins where they are out of phase            (cos < 0) → negative mask
  → subtracts energy that's phase-opposed to the target
  → equivalent to soft phase correction without predicting phase

The SI-SDR waveform loss drives the model to learn this naturally —
no change to the loss function, no explicit phase supervision.

Everything else (graph architecture, neighbour log-mag input,
bottleneck FiLM, dataset, training loop) is identical to V8.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Example (graph ON):
  CUDA_VISIBLE_DEVICES=0 python train_graph_psm_v9.py \\
    --data /home/rrame12/Desktop/Datasets/RestaurantSim_v4_m1_varspk \\
    --out  /home/rrame12/Desktop/Research/ASN/runs_psm_v9_on \\
...
Example (graph OFF / strict local-only baseline):
  CUDA_VISIBLE_DEVICES=0 python train_graph_psm_v9.py \\
    --data /home/rrame12/Desktop/Datasets/RestaurantSim_v4_m1_varspk \\
    --out  /home/rrame12/Desktop/Research/ASN/runs_psm_v9_off \\
    --batch 8 --epochs 120 --lr 3e-4 --patience 20 \\
    --crop_s 2.0 --nfft 1024 --hop 256 --num_workers 6 \\
...
class GraphAllToAllV9(nn.Module):
    """
    V9 (PSM, n_fft=1024) + all-to-all adjacency override.
    ...
    def __init__(
        ...
        self.adj_mode  = "alltoall"  # always alltoall for this class
    ...
    def forward(self, Y: torch.Tensor, A: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        # V9: tanh instead of sigmoid → PSM range [-1, 1]
        # Negative values subtract energy from phase-opposed bins
        mask = torch.tanh(self.out_mask(h).squeeze(1))      # (BK, F, TT)
